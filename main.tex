...
\begin{equation*}
	\nabla f(\vec{x}) = \begin{bmatrix}
		\dfrac{\partial f(\vec{x})}{\partial x_0}\\[12pt]
...
\begin{itemize}
	...
	\item[3.] In order to match the dimensionality of $\vec{\theta}$, we do need a dimensional parameter $\eta$ so that
	$$ \Delta \vec{\theta} = -\eta \nabla C(\vec{\theta})$$
...
\end{itemize}
A note on $\eta$ parameters. Usually in ML applications it is considered adimensional, since we are interested more on the "empirical" consequences rather than dimension considerations. However, ideally we should consider $\left[ \eta \right] = \dfrac{\left[ \theta \right]^2}{\left[ C \right]}$.
...
This section will only be a very quick description of all basic types of neural networks. Any neural network is a structure that process an input $\vec{x}\in \mathbb{R}^d$ of and returns an output $\vec{y}\in \mathbb{R}^p$.

The input is processed by "layers" which are column vectors with variable parameters and then the output is modified by a non-linear function called activation function. Each value of the layer is called "node", each node is a parameter of the neural network.
In general, each node contains $n$ weights and one bias, for instance in a fully connected network, every node contains as many parameters as the dimension of the input vector plus one bias.
...
\begin{center}
	...
	\begin{tikzpicture}[
		...
		% Hidden layer
		\node[neuron, right=28mm of x1] (h1) {$h_1$};
		\node[neuron, below=8mm of h1] (h2) {$h_2$};
		\node[neuron, below=8mm of h2] (h3) {$h_3$};
		\node[neuron, below=8mm of h3] (hm) {$h_m$};
		
		% Activation function: single circle after the hidden layer
		\node[
		activation,
		right=25mm of h2
		] (sigma) {$\sigma$};
		...
		% Output layer
		\node[neuron, right=25mm of sigma.north east] (y1) {$y_1$};
		\node[neuron, below=8mm of y1] (y2) {$y_2$};
		\node[neuron, below=8mm of y2] (yp) {$y_p$};
		...
		% All hidden-layer nodes connect to the activation function
		\foreach \i in {h1,h2,h3,hm}
		\draw[connection] (\i) -- (sigma);
		
		% Activation function connects to the output layer
		\foreach \j in {y1,y2,yp}
		\draw[connection] (sigma) -- (\j);
		
		% Layer labels
		\node[layer label, above=7mm of x1] {Input layer};
		\node[layer label, above=7mm of h1] {Hidden layer};
		\node[layer label, above=7mm of y1] {Output layer};
		
		% Mathematical labels
		\node[above=18mm of x1] {$\vec{x}\in\mathbb{R}^{d}$};
		\node[above=18mm of y1] {$\vec{y}\in\mathbb{R}^{p}$};
		
		% Activation label
		\node[below=8mm of sigma] {\small activation function};
...
Let $\vec{x}\in \mathbb{R}^d$ an input feature vector and let it contains $M$ layers. Each layer $l$ contains $W^{(l)}$ parameters, with $W^{(l)}$ being the weight matrix of the $l$-th layer -- could be a vector or matrix.
...
$$
	\vec{h}_{l+1} = \sigma_l(W^{(l)} \vec{x}_l +\vec{b}_l)\;,
$$
