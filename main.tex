...
\begin{equation*}
	A_{ij}v_j \rightarrow \sum_{j=1}^{n} A_{ij}v_j.
\end{equation*}
This is simply the $i$-th component of the matrix-vector multiplication:
\begin{equation*}
	(Av)_i = A_{ij}v_j.
\end{equation*}
Similarly, the inner product between two vectors $v,u\in\mathbb{R}^n$ is written as:
\begin{equation*}
	v_i u_i \rightarrow \sum_{i=1}^{n} v_i u_i.
\end{equation*}
Any index appearing once is a called "free index", while an index appearing twice is a \emph{summed index}. For example, in:
\begin{equation*}
	w_i = A_{ij}v_j,
\end{equation*}
the index $j$ is summed over, whereas $i$ is free. This means that the equation defines each component $w_i$ of the resulting vector.

In tensor notation, the position of an index matters. An index written below, as in $v_i$, is called a "lower index", while an index written above, as in $v^i$, is called an "upper index".

Upper indices are typically used for the components of vectors. Lower indices are typically used for the components of covectors, meaning linear maps that take a vector as input and return a scalar. For this reason, expressions such as
\begin{equation*}
	\omega_i v^i
\end{equation*}
have a natural meaning: a covector $\omega_i$ acts on a vector $v^i$, and the repeated index implies summation. In particular,
\begin{equation*}
	\omega_i v^i = \omega(\vec{v}),
\end{equation*}
that is, the covector $\omega$ acts on the vector $\vec{v}$ and produces a scalar. In Euclidean space, once the metric is used to identify vectors and covectors, this operation can be written as the usual scalar product: if $\vec{v}\in\mathbb{R}^N$ and $\omega$ is a covector, with associated vector $\vec{\omega}\in\mathbb{R}^N$, then
\begin{equation*}
...
The metric tensor, usually denoted by $g_{ij}$, allows us to pass from vectors to covectors and back. More precisely, if $v^j$ are the components of a vector, then the corresponding covector with lowered index is defined by
\begin{equation*}
	v_i = g_{ij}v^j.
\end{equation*}
...
\right|.
$$
A large $\Delta_\tau$ suggests a structural break at $\tau$; more sophisticated methods test many candidate points and/or other statistics (variance, slope) rather than just the mean.
