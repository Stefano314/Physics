...
PCA is a dimensionality reduction technique that is based on projecting the data into the eigenvectors associated to the largest eigenvalues.

Let $X$ be the $n\times m$ matrix data -- $n$ points, $m$ features -- to which we subtracted to each column its own mean, so that each column's mean is zero. ...
