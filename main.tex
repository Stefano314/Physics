...
The mathematical framework for RL is based on a \textit{Markov Decision Process} (MDP), represented by the tuple $(S,A,P,R,\gamma)$. $S$ is the set of states the system can assume -- terrain coordinates, pieces disposition on the board
$$
...
$$
\mathcal{R}(s,a,u) = \mathbb{E}\left[R_{t+1}\mid S_t=s,\, A_t=a,\, S_{t+1}=u\right] \;,
$$
and $\gamma$ is the factor weighting future rewards. The behavior of the agent in the environment is described by the \textit{policy} $\pi$, which describes the probability that the agent will choose a specific action given the current state
$$
\pi(a \mid s) = \Pr(A_t=a \mid S_t=s) \;.
$$
It important to notice that the \textit{reward} $R$ is an expected value, since it can be deterministic or stochastic. A car on a track will have a deterministic reward -- the lower the time, the better the reward --, while a more natural environment could have probabilities -- probability to get ill, or whatever.
