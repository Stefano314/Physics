...
$$
is the maximum expected return obtainable when the agent starts in a state $s$, selects action $a$ and then behaves optimally. Once $Q^*(s,a)$ is known, the best policy for a state is found as:
$$
\pi^*(s) = \arg \max_{a\in A} Q^*(s,a)\;,
$$
so we maximize over actions now instead of policies as before.

Note that we wrote down the equations considering generic stochastic setups, but for deterministic tasks we would remove all the uncertainties related to state transitions; however, every setup will contain some sort of unpredictability in order to explore all the possibilities, even the ones that should not be happen normally.
