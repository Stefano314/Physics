...
Let $V$ be an $N$-dimensional vector space over a field $\mathbb{F}$.
The tensor product is a bilinear map
\begin{equation*}
	\otimes : V\times V \longrightarrow V\otimes V,
\end{equation*}
defined by
\begin{equation*}
	(\vec{u},\vec{v})\longmapsto \vec{u}\otimes\vec{v}.
\end{equation*}
Let $\{\vec{e}_1,\ldots,\vec{e}_N\}$ be a basis of $V$ (canonical basis), and write
\begin{equation*}
	\vec{u}=\sum_{i=1}^{N}u_i\vec{e}_i,
	\qquad
	\vec{v}=\sum_{j=1}^{N}v_j\vec{e}_j.
\end{equation*}
By bilinearity, their tensor product is
\begin{equation*}
	\vec{u}\otimes\vec{v}
	=
	\sum_{i=1}^{N}\sum_{j=1}^{N}
	u_i v_j\,\vec{e}_i\otimes\vec{e}_j.
\end{equation*}
The tensors
\begin{equation*}
	\{\vec{e}_i\otimes\vec{e}_j\}_{1\leq i,j\leq N}
\end{equation*}
form a basis of $V\otimes V$. Therefore,
\begin{equation*}
	\dim(V\otimes V)=N^2.
\end{equation*}

Once a basis is chosen, the tensor $\vec{u}\otimes\vec{v}$ can be
...
In matrix notation, this is the outer product
\begin{equation*}
	[\vec{u}\otimes\vec{v}]
	=
	\vec{u}\vec{v}^{\top}.
...
More generally, an arbitrary tensor $\mathsf{T}\in V\otimes V$ has the
form
\begin{equation*}
	\mathsf{T}
	=
	\sum_{i=1}^{N}\sum_{j=1}^{N}
	T_{ij}\,\vec{e}_i\otimes\vec{e}_j,
\end{equation*}
and is represented, in the chosen basis, by the matrix
\begin{equation*}
	[\mathsf{T}]
	=
	\begin{bmatrix}
...
Thus, a simple tensor $\vec{u}\otimes\vec{v}$ is represented by a
rank-one outer-product matrix $\vec{u}\vec{v}^{\top}$, whereas a general
tensor in $V\otimes V$ is represented by an arbitrary $N\times N$ matrix.
